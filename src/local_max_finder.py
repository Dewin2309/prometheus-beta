...
def find_local_maxima(arr: List[int]) -> List[int]:
    ...
    A local maximum is defined as an element that is greater than both 
    its immediate neighbors. For the first and last elements, 
    they are considered local maxima if they are greater than their 
    ...
    Returns:
        List[int]: Indices of local maximum values in the array
